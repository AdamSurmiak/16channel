...
CMFCSample::CMFCSample(CWnd* pParent /*=NULL*/)
    ...
    , msg_thread_handle( 0 ) 
{
    // initializes the Bio Logic functions
    CString dll_path = TEXT("..\\..\\..\\..\\..\\EC-Lab Development Package\\EClib.dll");
    if( BL_Init(dll_path, eclib) != ERR_NOERROR ){
        CString msg;
        msg.Format(TEXT("An error occurred while loading the DLL.\nAre you sure it is located in '%s'?"), dll_path );
        // this error is fatal. Don't continue.
...
void CMFCSample::setupChannels(){
    ...
    int err = eclib->BL_GetChannelsPlugged( conn_id, channels, len );
    if( err == ERR_NOERROR ){
        CString number;
        ...
        err = eclib->BL_LoadFirmware( conn_id, channels, results, len, true, force_fw_reload, 0, 0 );
        if( err != ERR_NOERROR ) {
...
static int s_set_OcvParameters(TEccParams_t* params, TEClibFunctions* eclib, BOOL vmp4, CString &file, int xrec)
{
    ...
    file = vmp4 ? "ocv4.ecc" : "ocv.ecc";

    /* setup parameters */
    status |= eclib->BL_DefineSglParameter( "Rest_time_T",     10.0f, 0,    &param_list[0] );
    status |= eclib->BL_DefineSglParameter( "Record_every_dE",  0.1f, 0,    &param_list[1] );
    status |= eclib->BL_DefineSglParameter( "Record_every_dT", 0.01f, 0,    &param_list[2] );
    status |= eclib->BL_DefineIntParameter( "E_Range", KBIO_ERANGE_AUTO, 0, &param_list[3] );
    status |= eclib->BL_DefineIntParameter( "xctr",             xrec, 0,    &param_list[4] );
    return status;
}

/** This function sets up the parameters for a ChronoPotentiometry technique pass */
static int s_setCPParameters(TEccParams_t* params, TEClibFunctions* eclib, BOOL vmp4, CString &file, int xrec)
{
    int status = ERR_NOERROR;
    params->len = 17;
    TEccParam_t   *param_list = new TEccParam_t[params->len];
    ...
    params->pParams = param_list;
    file = vmp4 ? "cp4.ecc" : "cp.ecc";
    /* setup parameters */
    //{Step #0}
    status |= eclib->BL_DefineSglParameter ("Current_step",   0.002f,   0, &param_list[0]); //   {I0 (A)}
    status |= eclib->BL_DefineBoolParameter("vs_initial",     FALSE,    0, &param_list[1]); //   {vs. init}
    status |= eclib->BL_DefineSglParameter ("Duration_step",  0.1f,     0, &param_list[2]); //   {Step duration (s)}
    //{Step #1}
    status |= eclib->BL_DefineSglParameter ("Current_step",   -0.001f,  1, &param_list[3]); //   {I1 (A)}
    status |= eclib->BL_DefineBoolParameter("vs_initial",     FALSE,    1, &param_list[4]); //   {scan to E1 s. init}
    status |= eclib->BL_DefineSglParameter ("Duration_step",  0.2f,     1, &param_list[5]); //   {Step duration (s)
    //{Step #2}
    status |= eclib->BL_DefineSglParameter ("Current_step",   0.004f,   2, &param_list[6]); //   {I2 (A)}
    status |= eclib->BL_DefineBoolParameter("vs_initial",     FALSE,    2, &param_list[7]); //   {vs. init}
    status |= eclib->BL_DefineSglParameter ("Duration_step",  0.1f,     2, &param_list[8]); //   {Step duration (s)}
    //{others}
    status |= eclib->BL_DefineIntParameter ("Step_number",    2,        0, &param_list[9]); //   {step number}
    status |= eclib->BL_DefineIntParameter ("N_Cycles",       0,        0, &param_list[10]); //  {cycle Nc time}
    status |= eclib->BL_DefineSglParameter ("Record_every_dE",0.1f,     0, &param_list[11]); //  {record every dE (V)}
    status |= eclib->BL_DefineSglParameter ("Record_every_dT",0.01f,    0, &param_list[12]); //  {or every dT (s)}
    status |= eclib->BL_DefineIntParameter ("I_Range",        KBIO_IRANGE_10mA, 0, &param_list[13]); //  {I Range}
    status |= eclib->BL_DefineIntParameter ("E_Range",        KBIO_ERANGE_AUTO, 0, &param_list[14]); //  {E Range}
    status |= eclib->BL_DefineIntParameter ("Bandwidth",      KBIO_BW_5, 0, &param_list[15]); //  {bandwidth}
    status |= eclib->BL_DefineIntParameter( "xctr",             xrec, 0,    &param_list[16] );
    return status;
}

/** This function sets up the parameters for a ChronoAmperometry technique pass */
static int s_setCAParameters(TEccParams_t* params, TEClibFunctions* eclib, BOOL vmp4, CString &file, int xrec)
{
    int status = ERR_NOERROR;
    params->len = 17;
    TEccParam_t   *param_list = new TEccParam_t[params->len];
    ...
    params->pParams = param_list;
    file = vmp4 ? "ca4.ecc" : "ca.ecc";
    //{Step #0}
    status |= eclib->BL_DefineSglParameter ("Voltage_step",   1.5f,  0, &param_list[0]); //   {E0 (V)}
    status |= eclib->BL_DefineBoolParameter("vs_initial",     FALSE, 0, &param_list[1]); //   {vs. init}
    status |= eclib->BL_DefineSglParameter ("Duration_step",  0.1f,  0, &param_list[2]); //   {Step duration (s)}
    //{Step #1}
    status |= eclib->BL_DefineSglParameter ("Voltage_step",   -1.0f, 1, &param_list[3]); //   {E1 (V)}
    status |= eclib->BL_DefineBoolParameter("vs_initial",     FALSE, 1, &param_list[4]); //   {scan to E1 s. init}
    status |= eclib->BL_DefineSglParameter ("Duration_step",  0.2f,  1, &param_list[5]); //   {Step duration (s)
    //{Step #2}
    status |= eclib->BL_DefineSglParameter ("Voltage_step",   2.0f,  2, &param_list[6]); //   {E2 (V)}
    status |= eclib->BL_DefineBoolParameter("vs_initial",     FALSE, 2, &param_list[7]); //   {vs. init}
    status |= eclib->BL_DefineSglParameter ("Duration_step",  0.1f,  2, &param_list[8]); //   {Step duration (s)}
    //{others}
    status |= eclib->BL_DefineIntParameter ("Step_number",    2,     0, &param_list[9]); //   {step number}
    status |= eclib->BL_DefineIntParameter ("N_Cycles",       0,     0, &param_list[10]); //  {cycle Nc time}
    status |= eclib->BL_DefineSglParameter ("Record_every_dI",0.1f,  0, &param_list[11]); //  {record every dI (A)}
    status |= eclib->BL_DefineSglParameter ("Record_every_dT",0.01f, 0, &param_list[12]); //  {or every dT (s)}
    status |= eclib->BL_DefineIntParameter ("I_Range",        KBIO_IRANGE_AUTO, 0, &param_list[13]); //  {I Range}
    status |= eclib->BL_DefineIntParameter ("E_Range",        KBIO_ERANGE_AUTO, 0, &param_list[14]); //  {E Range}
    status |= eclib->BL_DefineIntParameter ("Bandwidth",      KBIO_BW_5, 0, &param_list[15]); //  {bandwidth}
    status |= eclib->BL_DefineIntParameter( "xctr",             xrec, 0,    &param_list[16] );
    return status;
...
int CMFCSample::insertOCVData( const TDataBuffer_t& dbuf, const TDataInfos_t& inf, const TCurrentValues_t& curr )
{
    ...
    if( inf.NbRows != 0 && inf.NbCols >= 3 ){ /* we expect 3 rows for vmp4 devices, 4 for vmp3 */
        for( int i=0; i< inf.NbRows; i++ ){
            ...
            FLOAT  f_ewe  = 0.0f;
            CString str;
            status = eclib->BL_ConvertNumericIntoSingle( ewe, &f_ewe );
            if( status == ERR_NOERROR ){
                ...
                if( inf.NbCols > 3 ){
                    ...
                    for( int k = 3; k<inf.NbCols; k++){
                        ...
                        status = eclib->BL_ConvertNumericIntoSingle( additional, &f_additional );
                        if( status == ERR_NOERROR ){
...
int CMFCSample::insertChronoData( const TDataBuffer_t& dbuf, const TDataInfos_t& inf, const TCurrentValues_t& curr )
{
    ...
    if( inf.NbRows != 0 && inf.NbCols >= 5 ){ /* we expect 5 rows for CP and CA */
        for( int i=0; i< inf.NbRows; i++ ){
            ...
            status  = eclib->BL_ConvertNumericIntoSingle( ewe, &f_ewe );
            status |= eclib->BL_ConvertNumericIntoSingle( I, &f_I );
...
UINT CMFCSample::populateData( PVOID data ){
    ...
    while( !stop ){
        ...
        status = work->eclib->BL_GetData( work->conn_id, work->channel, &tdata->buf, &tdata->infos, &tdata->curr );
        if( status == ERR_NOERROR ) {
            ...
            FromHandle( work->hwnd )->PostMessage( UWM_UPDATE_RESULTS, 0, (LPARAM)tdata);
             ...
             if( tdata->curr.State != KBIO_STATE_RUN || *(work->stop) ) {
                status |= work->eclib->BL_StopChannel( work->conn_id, work->channel);
                stop = true;
            }
        } else {
...
UINT CMFCSample::getBLMessages( PVOID data ){
    ...
    while( !(*work->stop) ){
        memset( msg, 0, 1024 );
        msg_len = 1024;
        status = work->eclib->BL_GetMessage( work->conn_id, work->channel, msg, &msg_len );
...
void CMFCSample::OnConnectClicked()
{
    ...
    log(L"Connect to %s\n", ip);
    if( !ip.IsEmpty() ){
        if( eclib->BL_Connect( CStringA(ip), 10, &conn_id, &infos ) != ERR_NOERROR ){
            // error
...
void CMFCSample::OnDisconnectClicked()
{
    if( conn_id != -1 ){
        ...
        if( eclib->BL_Disconnect( conn_id ) != ERR_NOERROR )
...
void CMFCSample::OnChanInfoClicked()
{
    ...
    int err = eclib->BL_GetChannelInfos( conn_id, c, &cinfos );
    if( err == ERR_NOERROR ) {
        CString msg;
...
void CMFCSample::OnChanCurrentValueClicked()
{
    ...
    int err = eclib->BL_GetCurrentValues( conn_id, c, &cvalues );
    if( err == ERR_NOERROR ) {
        CString msg;
...
void CMFCSample::OnStartClicked()
{
    ...
    setupDataList( technique, vmp4, xrec );
     if( technique == "OCV" ){ 
        status = s_set_OcvParameters(&params, eclib, vmp4, tech_file, xrec);
    } else if (technique == "ChronoPotentiometry" ) {
        status = s_setCPParameters(&params, eclib, vmp4, tech_file, xrec);
    } else if (technique == "ChronoAmperometry" ) {
        status = s_setCAParameters(&params, eclib, vmp4, tech_file, xrec);
    }
    ...
    if( status == ERR_NOERROR && params.len != 0 && tech_file[0] != '\0' && ch != -1 ){
        ...
        status = eclib->BL_LoadTechnique( conn_id, ch, CStringA(tech_file), params, true, true, show_pars );
        ...
        if( status != ERR_NOERROR ){
            ...
            if( eclib->BL_StartChannel( conn_id, ch ) == ERR_NOERROR ){
                // setup buttons                
