...
type
  TMainFrm = class(TForm)
    ...
    procedure cmboTechniqueChange(Sender: TObject);
...
begin
   ...
   0: {OCV}
      begin
         if FIsVMP4 then
            filename := LIB_DIRECTORY + 'ocv4.ecc' + #0
         else
            filename := LIB_DIRECTORY + 'ocv.ecc' + #0;
   ...
   1: {VSCAN}
      begin
         if FIsVMP4 then
            filename := LIB_DIRECTORY + 'vscan4.ecc' + #0
         else
            filename := LIB_DIRECTORY + 'vscan.ecc' + #0;
   ...
   2: {CP}
      begin
         if FIsVMP4 then
            filename := LIB_DIRECTORY + 'cp4.ecc' + #0
         else
            filename := LIB_DIRECTORY + 'cp.ecc' + #0;
   ...
   3: {CA}
      begin
         if FIsVMP4 then
            filename := LIB_DIRECTORY + 'ca4.ecc' + #0
         else
            filename := LIB_DIRECTORY + 'ca.ecc' + #0;
         SetLength(EccParamArray, 16);
         {Step #0}
   ...
   4: {MOD}
      begin
         if FIsVMP4 then
            filename := LIB_DIRECTORY + 'mp4.ecc' + #0
         else
            filename := LIB_DIRECTORY + 'mp.ecc' + #0;
         ...
         begin
            ...
            begin
               ...
               begin
                  ...
                  StringGrid1.Cells[k + 1, indx] := StringReplace(Format('%.3e', [val_sgl]), '.', FormatSettings.DecimalSeparator, [rfReplaceAll]);
               end;
            end;
...
procedure TMainFrm.cmboTechniqueChange(Sender: TObject);
begin
